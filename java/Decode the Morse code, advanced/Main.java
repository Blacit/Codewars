...
public class MorseCodeDecoder
{
  ...
  public static final String BIT_LETTER_PART_DOT =
    repeat (BIT_ON, BIT_LETTER_PART_DOT_LENGTH);
  public static final String BIT_LETTER_PART_DASH =
    repeat (BIT_ON, BIT_LETTER_PART_DASH_LENGTH);
  ...
  public static final String BIT_LETTER_PART_PAUSE =
    repeat (BIT_OFF, BIT_LETTER_PART_PAUSE_LENGTH);
  public static final String BIT_LETTER_PAUSE =
    repeat (BIT_OFF, BIT_LETTER_PAUSE_LENGTH);
  public static final String BIT_WORD_PAUSE =
    repeat (BIT_OFF, BIT_WORD_PAUSE_LENGTH);
  ...
  public static final String MORSE_LETTER_DELIMITER =
    repeat (MORSE_DELIMITER, MORSE_LETTER_DELIMITER_LENGTH);
  public static final String MORSE_WORD_DELIMITER =
    repeat (MORSE_DELIMITER, MORSE_WORD_DELIMITER_LENGTH);
  ...
  public static String decodeBits (final String bits)
  {
	...
	final StringBuilder wordBuilder = new StringBuilder ();
	  ...
	  for (final String bitLetterPart:bitLetter.
		 split
		 (BIT_LETTER_PART_PAUSE))
	      {
		// deliberate cast to fail fast with NPE instead of appending "null"
		letterBuilder.append ((char) BIT_TO_MORSE.
				      get (bitLetterPart));
	      }
  ...
  for (final String morseWord:morseCode.trim ().
  ...
  private static String normalize (final String bits)
  {
    ...
    return transmissionRate <= 1
      ? trimmedBits
      : loopThroughBits (trimmedBits, transmissionRate,
			 new StringBuilder (trimmedBits.length () /
					    transmissionRate),
			 MorseCodeDecoder::repeat,
  ...
  private static int getTransmissionRate (final String bits)
  {
    return bits.isEmpty ()? 0
      : loopThroughBits (bits, 1, new HashSet < Integer > (),
			 (lastBit, bitCount)->bitCount,
			 Collection::add).stream ().
      mapToInt (Integer::intValue).reduce (MorseCodeDecoder::gcd).getAsInt ();
  }

  private static <C, E > C loopThroughBits (final String bits, final int step,
					    final C container,
					    final BiFunction < Character,
					    Integer, E > elementProducer,
					    final BiConsumer < C,
					    E > appender)
  {
	...
	if (bit == lastBit)
	  ...
	  {
	    appender.accept (container,
			     elementProducer.apply (lastBit, bitCount));
	    bitCount = 1;
	    lastBit = bit;
	  }
      }
